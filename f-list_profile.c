...
void flist_get_profile(PurpleConnection *pc, const char *who) {
    ...
    if(!character) {
        ...
        flp->profile_request = flist_web_request(JSON_CHARACTER_INFO, args, NULL, TRUE, flist_get_profile_cb, fla);
...
void flist_profile_load(FListAccount *fla) {
    ...
    flp->global_profile_request = flist_web_request(JSON_INFO_LIST, NULL, NULL, TRUE, flist_global_profile_cb, fla);
